...
const SignUp = () => {
  ...
  const handleSubmit = async(e) => {
    ...
    const respo3=await axios.post(`${baseurl}/api/users/otpverify`,{email:formData.email,otp:formData.otp,name:formData.name})
    ...
    loaderdispatcher({type:"FETCH_STARTED",payload:"fetching started"})
    const respo=await axios.post(`${baseurl}/api/users/signup`, formData)
    loaderdispatcher({type:"FETCH_SUCCESS",payload:"fetching completed"})
    if(respo.data.ok){
  ...
  const googleLogin = useGoogleLogin({
    ...
    if(data){
      try{
      loaderdispatcher({type:"FETCH_STARTED",payload:"fetching started"})
      const respo=await axios.post(`${baseurl}/api/users/googleauth`,data)
      loaderdispatcher({type:"FETCH_SUCCESS",payload:"fetching completed"})
      if (respo.data.ok){
  ...
  const otpRequestHandler=async()=>{
    try{
        ...
        loaderdispatcher({type:"FETCH_STARTED",payload:"fetching started"})
        const respo=await axios.post(`${baseurl}/api/users/getotp`,{email:formData.email,name:formData.name})
        loaderdispatcher({type:"FETCH_SUCCESS",payload:"fetching completed"})
        if(respo.data.ok){
            setOtpform(true)
